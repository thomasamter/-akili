// AKILI Signup Page
// Create new account with email or social login

import { useState } from 'react'
import { useNavigate, Link } from 'react-router-dom'
import { motion } from 'framer-motion'
import { Mail, Lock, User, Eye, EyeOff, Brain } from 'lucide-react'
import { signUpWithEmail, signInWithGoogle, signInWithApple } from '../lib/firebase'
...
const SignupPage = () => {
  ...
  const handleEmailSignup = async (e) => {
    ...
    const { user, error: authError } = await signUpWithEmail(email, password, displayName)
    ...
    if (user) {
      login({ id: user.uid, email: user.email, displayName }, user.accessToken)
      navigate('/')
    }
    ...
    <div className="min-h-screen bg-akili-black flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 py-8">
        ...
        >
          ...
          <p className="text-white/60">Join the AKILI community today</p>
        </motion.div>
        ...
        <motion.p
          ...
          className="mt-6 text-white/40 text-xs text-center max-w-sm"
        >
          By creating an account, you agree to our{' '}
          <a href="#" className="text-akili-gold">Terms of Service</a> and{' '}
          <a href="#" className="text-akili-gold">Privacy Policy</a>
        </motion.p>
        ...
        >
          ...
          <Link to="/login" className="text-akili-gold font-medium hover:text-akili-gold/80">
            Sign in
          </Link>
        </motion.p>
      </main>
